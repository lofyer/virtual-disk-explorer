...
$root_path = $_SERVER['DOCUMENT_ROOT'].'/vde';
...
// vde: Mount
if (isset($_GET['mount']) && !FM_READONLY) {
    $mount = $_GET['mount'];
    $mount = fm_clean_path($mount);
    ...
    if ($mount != '' && is_file($path . '/' . $mount)) {
        $mount_path = $path . '/' . $mount;
        $ext = pathinfo($mount_path, PATHINFO_EXTENSION);
        $isValid = true;
    } else {
        fm_set_msg('File not found', 'error');
    }
    $mount_path_dir = $mount_path . "_mount";
    mkdir($mount_path_dir);
    $command_guestmount = "export LIBGUESTFS_BACKEND=direct;guestmount -a $mount_path -i $mount_path_dir;";
    $output = system($command_guestmount);
    fm_set_msg($output);
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}

// vde: UnMount
if (isset($_GET['unmount']) && !FM_READONLY) {
    $mount = $_GET['unmount'];
    $mount = fm_clean_path($mount);
    $mount = str_replace('/', '', $mount);
    $isValid = false;
    ...
    $mount_path_dir = $mount_path . "_mount";
    $command_guestmount = "export LIBGUESTFS_BACKEND=direct;umount $mount_path_dir";
    system($command_guestmount);
    sleep(5);
    rmdir($mount_path_dir);
    fm_set_msg('Virtual Disk UnMounted!');
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['view'])) {
    ...
    } elseif ($ext == 'qcow2' || $ext == 'raw') { // for vde
        $is_virtualDisk= true;
        $view_title = 'Virtual Disk';
        ...
        <div class="col-12">
            <?php if(!$quickView) { ?>
                ...
                <p>
                    ...
                    <b><a href="<?php echo fm_enc($dir_path . $file_url) ?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php echo lng('Open') ?></a></b>
                    &nbsp;
                    <?php
                    ...
                    // vde: virtualDisk actions
                    if (!FM_READONLY && ($is_virtualDisk)) {
                        $virtual_disk_name = pathinfo($file_path, PATHINFO_FILENAME);
                        ?>
                        <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;mount=<?php echo urlencode($file) ?>"><i class="fa fa-check-circle"></i> <?php echo lng('Mount') ?></a></b> &nbsp;
                        <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unmount=<?php echo urlencode($file) ?>" title="UnMount<?php echo fm_enc($virtual_disk_name) ?>"><i class="fa fa-check-circle"></i>
                                <?php echo lng('Unmount') ?></a></b> &nbsp;
                        <?php
                    }
		    // Text actions
...
function lng($txt) {
    ...
    $tr['en']['Mount'] = 'Mount Virtual Disk';
    $tr['en']['UnMount'] = 'UnMount Virtual Disk';
